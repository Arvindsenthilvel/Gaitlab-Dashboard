...
.pill-sig    { background:rgba(63,185,80,0.15);  color:#3fb950; border-radius:10px; padding:2px 8px; font-size:11px; font-weight:700; }
.pill-ns     { background:rgba(125,133,144,0.12);color:#7d8590; border-radius:10px; padding:2px 8px; font-size:11px; }
.pill-trend  { background:rgba(227,179,65,0.15); color:#e3b341; border-radius:10px; padding:2px 8px; font-size:11px; font-weight:700; }
.pill-huge   { background:rgba(188,140,255,0.18);color:#bc8cff; border-radius:10px; padding:2px 8px; font-size:11px; font-weight:700; }
.pill-large  { background:rgba(88,166,255,0.15); color:#58a6ff; border-radius:10px; padding:2px 8px; font-size:11px; font-weight:700; }
...
@st.cache_data
def load_data():
    with open("subjects_data.json") as f:
...
def df_subjects():
    return pd.DataFrame(SUBJECTS_RAW)
...
if pg == "Overview":
    ...
    with col_r:
        ...
        fig.update_layout(**PLOT_LAYOUT, height=300,
                          yaxis_title="% Change", xaxis_tickangle=-35)
    ...
    with col_a:
        ...
        fig2 = go.Figure(go.Pie(
            ...
            values=[7, 2, 2],
            marker_colors=[GREEN, AMBER, MUTED],
            hole=0.4,
            textinfo="label+percent",
            textfont_size=10,
        ))
        fig2.update_layout(**{k:v for k,v in PLOT_LAYOUT.items() if k!="xaxis" and k!="yaxis"},
    ...
    with col_b:
        ...
        evals = []
        for k in pks:
            m = _re.findall(r"[\d.]+", STATS[k]["effect"])
            evals.append(abs(float(m[0])) if m else 0)
        fig3 = go.Figure(go.Bar(
            y=lbls, x=evals, orientation="h",
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    st.markdown('<div class="section-header">Subjects (N=25)</div>', unsafe_allow_html=True)

    emg_names = set(s["name"] for s in SUBJECTS_RAW[:13])
    rows = []
    for s in SUBJECTS_RAW:
        cs = s.get("Walking_speed_ctrl"); ss = s.get("Walking_speed_smart")
        rows.append({
            "#": s["id"], "Name": s["name"], "Age": s["age"],
            "Height (cm)": s["height"], "Weight (kg)": s["weight"],
            "EMG": "✓" if s["name"] in emg_names else "—",
            "Speed Ctrl": round(cs,3) if cs else "—",
            "Speed Smart": round(ss,3) if ss else "—",
    ...
    sel_name = st.selectbox("Select a subject", [s["name"] for s in SUBJECTS_RAW])
    s = next(x for x in SUBJECTS_RAW if x["name"] == sel_name)

    col_bio, col_params = st.columns([1, 2])
    with col_bio:
        st.markdown(f"**{s['name']}** — Subject S{s['id']}")
        st.markdown(f"""
        | Field | Value |
        |-------|-------|
        | Age | {s['age']} yrs |
        ...
        | Weight | {s['weight']} kg |
        | Gender | {s['gender']} |
        | EMG | {"✓ Available" if s['name'] in emg_names else "— Not available"} |
        """)
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    with tab1:
        ...
        fig.add_trace(go.Bar(name="Control",    x=lbls, y=[STATS[k]["ctrl"] for k in pks],
                             marker_color=BLUE, opacity=0.85, width=0.35,
                             offset=-0.18, text=[STATS[k]["ctrl"] for k in pks], textposition="outside"))
        fig.add_trace(go.Bar(name="Smartphone", x=lbls, y=[STATS[k]["smart"] for k in pks],
                             marker_color=RED,  opacity=0.80, width=0.35,
                             offset=0.18,  text=[STATS[k]["smart"] for k in pks], textposition="outside"))
    ...
    with tab2:
        ...
        names = [short_name(s) for s in valid]
        fig2 = go.Figure()
        fig2.add_trace(go.Scatter(x=names, y=[s[pk_sel+"_ctrl"] for s in valid],
                                   mode="markers", name="Control",
                                   marker=dict(color=BLUE, size=9, symbol="circle")))
        fig2.add_trace(go.Scatter(x=names, y=[s[pk_sel+"_smart"] for s in valid],
        ...
        fig2.update_layout(**PLOT_LAYOUT, height=380,
                            yaxis_title=STATS[pk_sel]["unit"],
                            title=f"{STATS[pk_sel]['label']} — Individual Values (dashed = group mean)")
        st.plotly_chart(fig2, use_container_width=True)

    with tab3:
        rows = []
        for s in SUBJECTS_RAW:
            row = {"#": s["id"], "Name": s["name"]}
            for pk in pks:
                cv = s.get(pk+"_ctrl"); sv = s.get(pk+"_smart")
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    for i, pk in enumerate(pks):
        s = STATS[pk]
        col_d = "normal" if s["sig"] else "off"
        cols[i].metric(s["label"], f"{s['ctrl']} {s['unit']}",
    ...
    <b>Ankle Dorsiflexion Paradox:</b> Despite walking 16% slower (where DF normally decreases at lower speeds),
    ankle DF significantly <b>increased</b> 8.1% (p=0.005). Hip and knee were completely unchanged (p=0.51, p=0.67).
    This selective distal CNS compensation is the key novel kinematic finding of this study.
    </div>
    """, unsafe_allow_html=True)
    ...
    with tab2:
        ...
        for i, pk in enumerate(pks):
            ...
            names = [short_name(s) for s in valid]
            fig2.add_trace(go.Scatter(x=names, y=[s[pk+"_ctrl"] for s in valid],
                                       mode="markers", name="Control" if i==0 else "",
                                       marker=dict(color=PURPLE, size=7),
                                       showlegend=(i==0)), row=1, col=i+1)
            fig2.add_trace(go.Scatter(x=names, y=[s[pk+"_smart"] for s in valid],
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    <b>Net Impulse Sign Reversal:</b> Control net impulse: <b>+0.9 N·s</b> (net-propulsive) →
    Smartphone: <b>−0.7 N·s</b> (net-decelerative). The Prop:Braking ratio drops from 1.051 → 0.956,
    crossing below 1.0. This mechanistically explains the speed reduction — only detectable with force plates.
    </div>
    """, unsafe_allow_html=True)
    ...
    with tab2:
        ...
        fig2 = go.Figure()
        fig2.add_trace(go.Bar(name="Control",    x=ns, y=pbC, marker_color=GREEN, opacity=0.82, width=0.35, offset=-0.18))
        fig2.add_trace(go.Bar(name="Smartphone", x=ns, y=pbS, marker_color=RED,   opacity=0.78, width=0.35, offset=0.18))
        ...
        fig2.update_layout(**PLOT_LAYOUT, height=380, barmode="overlay",
                           yaxis_title="Propulsion:Braking Ratio",
                           title="P:B Ratio — values <1.0 = net-decelerative gait")
        st.plotly_chart(fig2, use_container_width=True)
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    st.markdown('<div class="study-tag">n=13 subjects with complete EMG · Delsys Trigno · Gastrocnemius Medial & Lateral (Left & Right)</div>', unsafe_allow_html=True)
    ...
    c1,c2,c3,c4 = st.columns(4)
    c1.metric("EMG Subjects","13","of 25 total · 52%")
    ...
    <b>Underpowered result:</b> With n=13, power is only ~38% for a medium effect size (d=0.5).
    The directional EMG increase alongside propulsion deficit suggests neuromuscular inefficiency —
    more activation producing less mechanical output under cognitive load. A larger study (n≥34) is needed
    to confirm this with adequate statistical power.
    </div>
    """, unsafe_allow_html=True)
    ...
    with tab1:
        ...
        fig = go.Figure()
        fig.add_trace(go.Bar(name="Control",    x=en, y=[s["EMG_RMS_ctrl"] for s in es],
                             marker_color=BLUE, opacity=0.85, width=0.35, offset=-0.18))
        fig.add_trace(go.Bar(name="Smartphone", x=en, y=[s["EMG_RMS_smart"] for s in es],
                             marker_color=PURPLE, opacity=0.80, width=0.35, offset=0.18))
        fig.update_layout(**PLOT_LAYOUT, height=360, barmode="overlay", yaxis_title="RMS (mV)")
        st.plotly_chart(fig, use_container_width=True)
    ...
    with tab2:
        ...
        fig2.add_trace(go.Scatter(
            ...
            text=[short_name(s) for s in es], textposition="top right",
        ...
        fig2.update_layout(**PLOT_LAYOUT, height=400,
                           ...
                           title="Points above y=x line = increased EMG under smartphone")
        st.plotly_chart(fig2, use_container_width=True)

    with tab3:
        emg_rows = [{"#":s["id"],"Name":s["name"],
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    with tab1:
        ...
        st.markdown("**Significant results highlighted in green (p < 0.05):**")
        sig_pks = [pk for pk,s in STATS.items() if s["sig"]]
        c1,c2 = st.columns(2)
        for i, pk in enumerate(sig_pks):
            s = STATS[pk]
    ...
    with tab2:
        ...
        st.caption("Non-normal distributions (p < 0.05) → Wilcoxon signed-rank test used instead of paired t-test")
    ...
    with tab3:
        ...
        lbls = [STATS[k]["label"] for k in pks]
        fig = go.Figure()
        fig.add_trace(go.Bar(name="Control CV%",    x=lbls, y=cvC, marker_color=BLUE,   opacity=0.85, width=0.35, offset=-0.18))
        fig.add_trace(go.Bar(name="Smartphone CV%", x=lbls, y=cvS, marker_color=RED,    opacity=0.80, width=0.35, offset=0.18))
        fig.update_layout(**PLOT_LAYOUT, height=380, barmode="overlay",
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    with tab1:
        ...
        for col, title, ck, sk, ylab in [
            (c1,"Walking Speed — all 25 subjects","Walking_speed_ctrl","Walking_speed_smart","m/s"),
            (c2,"Stride Length — all 25 subjects","Stride_length_ctrl","Stride_length_smart","m"),
        ]:
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=ns, y=[s[ck] for s in SUBJECTS_RAW], mode="lines+markers",
                                     name="Control", line=dict(color=BLUE,width=2), marker=dict(size=5)))
            fig.add_trace(go.Scatter(x=ns, y=[s[sk] for s in SUBJECTS_RAW], mode="lines+markers",
                                     name="Smartphone", line=dict(color=RED,width=2), marker=dict(size=5, symbol="triangle-up")))
            fig.update_layout(**PLOT_LAYOUT, height=300, yaxis_title=ylab, title=title, xaxis_tickangle=-40)
            col.plotly_chart(fig, use_container_width=True)

        c3, c4 = st.columns(2)
        for col, title, ck, sk, ylab in [
            (c3,"Double Support — all subjects","Double_support_ctrl","Double_support_smart","%"),
            (c4,"Stride Variability — all subjects","Stride_variability_ctrl","Stride_variability_smart","%"),
        ]:
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=ns, y=[s[ck] for s in SUBJECTS_RAW], mode="lines+markers",
                                     name="Control", line=dict(color=GREEN,width=2), marker=dict(size=5)))
            fig.add_trace(go.Scatter(x=ns, y=[s[sk] for s in SUBJECTS_RAW], mode="lines+markers",
                                     name="Smartphone", line=dict(color=AMBER,width=2), marker=dict(size=5, symbol="triangle-up")))
            fig.update_layout(**PLOT_LAYOUT, height=300, yaxis_title=ylab, title=title, xaxis_tickangle=-40)
            col.plotly_chart(fig, use_container_width=True)
    ...
    with tab2:
        ...
        with c1:
            ...
            fig.add_trace(go.Scatter(
                ...
                mode="markers+text", text=ns, textposition="top right",
            ...
            fig.update_layout(**PLOT_LAYOUT, height=400,
                              ...
                              title="Propulsion vs Braking Scatter")
            st.plotly_chart(fig, use_container_width=True)

        with c2:
            cats = ["Walk Speed","Stride Length","Cadence","Ankle DF","Propulsion"]
            ctrl_v = [100,100,100,100,100]
            smart_v = [83.5,87.9,95.5,108.1,81.8]
            fig2 = go.Figure()
    ...
    with tab3:
        for pk in PGROUPS["kin"]:
            fig = go.Figure()
            valid = [s for s in SUBJECTS_RAW if s.get(pk+"_ctrl") is not None]
            vn = [short_name(s) for s in valid]
            fig.add_trace(go.Bar(name="Control",    x=vn, y=[s[pk+"_ctrl"] for s in valid],
                                  marker_color=PURPLE, opacity=0.85, width=0.35, offset=-0.18))
            fig.add_trace(go.Bar(name="Smartphone", x=vn, y=[s[pk+"_smart"] for s in valid],
                                  marker_color=RED,    opacity=0.80, width=0.35, offset=0.18))
            fig.update_layout(**PLOT_LAYOUT, height=300, barmode="overlay",
                              yaxis_title="°", title=STATS[pk]["label"])
            st.plotly_chart(fig, use_container_width=True)

    with tab4:
        pk_choice = st.selectbox("Select parameter to view all subjects",
                                  list(STATS.keys()), format_func=lambda k: STATS[k]["label"])
        valid = [s for s in SUBJECTS_RAW if s.get(pk_choice+"_ctrl") is not None]
        vn = [short_name(s) for s in valid]
        fig = go.Figure()
        fig.add_trace(go.Bar(name="Control",    x=vn, y=[s[pk_choice+"_ctrl"] for s in valid],
                              marker_color=BLUE, opacity=0.85, width=0.35, offset=-0.18))
        fig.add_trace(go.Bar(name="Smartphone", x=vn, y=[s[pk_choice+"_smart"] for s in valid],
                              marker_color=RED,  opacity=0.80, width=0.35, offset=0.18))
        fig.add_hline(y=STATS[pk_choice]["ctrl"],  line_dash="dot", line_color=BLUE,  line_width=1.5,
                      annotation_text=f"Group ctrl: {STATS[pk_choice]['ctrl']}")
        fig.add_hline(y=STATS[pk_choice]["smart"], line_dash="dot", line_color=RED,   line_width=1.5,
                      annotation_text=f"Group smart: {STATS[pk_choice]['smart']}")
        fig.update_layout(**PLOT_LAYOUT, height=400, barmode="overlay",
                          yaxis_title=STATS[pk_choice]["unit"],
                          title=f"{STATS[pk_choice]['label']} — All 25 Subjects")
        st.plotly_chart(fig, use_container_width=True)
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    search = col_f2.text_input("Search subject", placeholder="Name...")
    ...
    for s in SUBJECTS_RAW:
        ...
        row = {"#":s["id"], "Name":s["name"], "Age":s["age"], "Ht":s["height"], "Wt":s["weight"]}
        for pk in pks:
            if pk == "EMG_RMS" and s["name"] not in emg_names:
